...
const HistoryItem = React.memo(({ item }) => {
  ...
  const eventDate = item.event_date_utc
    ? new Date(item.event_date_utc).toUTCString()
    : "Date not available";
  ...
  return (
    <div
      key={item.id}
      className="p-4 border bg-[#292a2b6e] rounded-lg border-1 border-[#585858ab]"
    >
      <div>
        <span className="text-2xl font-extrabold">{title} - </span>
        <p
          style={{ fontFamily: "Montserrat" }}
          className="text-base font-medium mt-4 mb-6"
        >
          {details.slice(0, 100)}...
        </p>
      ...
      {articleLink ? (
        <div>
          ...
          {/* Iframe for displaying the article */}
          <iframe
            loading="lazy"
            src={articleLink}
            scrolling="no"
